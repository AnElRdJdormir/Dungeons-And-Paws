...
import { Modelos3D, objBunny, objWolf, objCoin, objCoinV, objCoinM } from './models.js';
const $ = window.jQuery;
...
function updateScore() {
    scoreElement.textContent = `Puntuación - Jugador 2: ${score.left} Jugador 1: ${score.right}`;
}
...
let intervalId = setInterval(() => {
    if (timeRemaining > 0) {
        ...
        showWinner();
        ballSpeed = 0; // Detener la pelota
    }
}, 1000);

function showWinner() {
    let winnerText = "";
    let winnerValue = 1; // Valor numérico que quieres pasar

    if (score.left > score.right) {
        winnerText = "El jugador 2 es el Ganador!";
    } else if (score.right > score.left) {
        winnerText = "El jugador 1 es el Ganador!";
    } else {
        winnerText = "Empate!";
    }
    ...
    // Redirigir a index.html con el valor numérico después de 5 segundos
    setTimeout(() => {
        window.location.href = `index.html?winner=${winnerValue}`;
    }, 5000);
}
